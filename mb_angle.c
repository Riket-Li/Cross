...
int mb_beaudoin(int verbose, mb_3D_orientation tx_align, mb_3D_orientation tx_orientation, double tx_steer,
  ...
  // Cross algorithm
  mb_3D_vector directionVect;
  directionVect.x = txGeo.y * rxGeo.z - txGeo.z * rxGeo.y;
  directionVect.y = txGeo.z * rxGeo.x - txGeo.x * rxGeo.z;
  directionVect.z = txGeo.x * rxGeo.y - txGeo.y * rxGeo.x;
  
  double denominator = (txGeo.x * rxGeo.y - rxGeo.x * txGeo.y);
  
  double x0 = (rxGeo.y * sin(tx_steer * DTR) + txGeo.y * sin(rx_steer * DTR)) / denominator;
  double y0 = -(txGeo.x * sin(rx_steer * DTR) + rxGeo.x * sin(tx_steer * DTR)) / denominator;
  
  double a = directionVect.x * x0 + directionVect.y * y0;
